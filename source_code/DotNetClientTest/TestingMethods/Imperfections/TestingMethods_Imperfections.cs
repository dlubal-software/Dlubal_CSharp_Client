    ...
    public static partial class TestingMethods
    {
            ...
            switch (type)
            {
                case imperfection_case_type.IMPERFECTION_TYPE_BUCKLING_MODE:
                {
                    imperfectionCase = new imperfection_case
                    {
                        ...
                        direction = imperfection_case_direction.IMPERFECTION_CASE_DIRECTION_GLOBAL_Z_OR_USER_DEFINED_W_TRUE,
                ...
                case imperfection_case_type.IMPERFECTION_TYPE_IMPERFECTION_CASES_GROUP:
                {
                    imperfectionCase = new imperfection_case
                    {
                        imperfection_cases_items = new imperfection_case_imperfection_cases_items_row[tableRowCount]
                ...
                case imperfection_case_type.IMPERFECTION_TYPE_INITIAL_SWAY_VIA_TABLE:
                {
                    imperfectionCase = new imperfection_case
                    {
                        ...
                        direction_for_level_direction = imperfection_case_direction_for_level_direction.DIRECTION_XY,
                        direction_for_level_directionSpecified = true,
                    ...
                    imperfectionCase.level_imperfections = new imperfection_case_level_imperfections_row[tableRowCount];

                    for (int i = 0; i < tableRowCount; i++)
                    {
                        imperfectionCase.level_imperfections[i] = new imperfection_case_level_imperfections_row
                        {
                            no = i + 1,
                            row = new imperfection_case_level_imperfections
                            {
                                level = i / 10.0,
        ...
        public static bool Test_Imperfections_Imperfection\u2040Cases_Create()
        {
            ...
            try
            {
                ...
                // get add-on statuses
                DataLogger.AddLogStart("Get add-on statuses...");
                addon_list_type addons = SoapModelClient.get_addon_statuses();
                ...
                SetImperfectionCase(IMPERFECTION_CASE_1_NO, imperfection_case_type.IMPERFECTION_TYPE_LOCAL_IMPERFECTIONS);
                SetImperfectionCase(IMPERFECTION_CASE_2_NO, imperfection_case_type.IMPERFECTION_TYPE_NOTIONAL_LOADS_FROM_LOAD_CASE);
                SetImperfectionCase(IMPERFECTION_CASE_3_NO, imperfection_case_type.IMPERFECTION_TYPE_STATIC_DEFORMATION);
                SetImperfectionCase(IMPERFECTION_CASE_4_NO, imperfection_case_type.IMPERFECTION_TYPE_IMPERFECTION_CASES_GROUP, 1);
                SetImperfectionCase(IMPERFECTION_CASE_5_NO, imperfection_case_type.IMPERFECTION_TYPE_INITIAL_SWAY_VIA_TABLE, 1);

                if (addons.analysis.structure_stability_active)
                {
                    SetImperfectionCase(IMPERFECTION_CASE_6_NO, imperfection_case_type.IMPERFECTION_TYPE_BUCKLING_MODE); // Add-on Structure Stability required
                }
                if (addons.dynamic_analysis_settings.modal_active)
                {
                    SetImperfectionCase(IMPERFECTION_CASE_7_NO, imperfection_case_type.IMPERFECTION_TYPE_DYNAMIC_EIGENMODE); // Add-on Modal Analysis required
                }
